...
export function intervalAleatório(min = 45_000, max = 80_000) {
...
export class BaileysManager {
    ...
    sockets = new Map();
    tentativas = new Map();
    prontoEm = new Map(); // contaId → timestamp de quando fica pronto
    foiConectado = new Map(); // contaId → chegou a abrir conexão 'open'
    ...
    async conectar(contaId) {
        ...
        const socket = makeWASocket({
            ...
            auth: {
                creds: state.creds,
                keys: makeCacheableSignalKeyStore(state.keys, logger.child({ contaId })),
            },
            browser: Browsers.macOS('Safari'), // fingerprint anti-ban
            printQRInTerminal: false,
            logger: logger.child({ contaId }),
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (qr) {
                ...
                await this.supabase
                    .from('conexoes')
                    .upsert({ conta_id: contaId, status: 'conectando', qr_code: qr, comando: null }, { onConflict: 'conta_id' });
            }
            if (connection === 'open') {
