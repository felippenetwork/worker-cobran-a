...
async function main() {
    ...
    const executarScheduler = async () => {
        try {
            ...
            logger.error({ err }, 'Scheduler: erro');
        }
    };
        ...
        while (true) {
            ...
            try {
                ...
                for (const row of pendentes ?? []) {
                    ...
                    const comando = row.comando;
                    logger.info({ contaId, comando }, 'Comando detectado via polling');
                    try {
                        if (comando === 'reconectar')
                            await manager.reconectar(contaId);
                        else if (comando === 'desconectar')
                            await manager.desconectar(contaId);
                    }
                    catch (err) {
    ...
    const loopWhatsApp = async () => {
        while (true) {
            try {
                await processarFilaWhatsApp(supabase, manager);
            }
            catch (err) {
                logger.error({ err }, 'Worker WA: erro');
            }
    ...
    const loopImediato = async () => {
        while (true) {
            try {
                await processarFilaImediata(supabase, manager);
            }
            catch (err) {
                logger.error({ err }, 'Worker imediato: erro');
            }
    ...
    const loopEmail = async () => {
        while (true) {
            try {
                await processarFilaEmail(supabase);
            }
            catch (err) {
                logger.error({ err }, 'Worker E-mail: erro');
            }
    ...
    const loopSyncConexoes = async () => {
        while (true) {
            ...
            try {
                await manager.sincronizarConexoes();
            }
            catch (err) {
                logger.error({ err }, 'Sync conexões: erro');
            }
        }
    };
    ...
    if (dpgpCronUrl) {
        const loopDpgp = async () => {
            while (true) {
                ...
                try {
                    await fetch(dpgpCronUrl);
                }
                catch { /* silencioso — não afeta este worker */ }
            }
        };
        loopDpgp();
